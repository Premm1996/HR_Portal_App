...
export default function AdminEmployees() {
  ...
  const handleApprove = async (id: number, employeeName: string) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/admin/employees/${id}/approve`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.ok) {
  ...
  const handleDecline = async (id: number, employeeName: string) => {
    if (!confirm(`Are you sure you want to decline ${employeeName}? This will delete the employee.`)) return;
    ...
    try {
      ...
      if (response.ok) {
        alert('Employee declined and deleted successfully');
        fetchEmployees();
      } else {
